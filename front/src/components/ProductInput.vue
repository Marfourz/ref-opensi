<template>
  <div
    class="border flex flex-col items-center rounded-lg px-4 py-6 space-y-4 relative"
    :class="{ 'border-success': isValidate }"
  >
    <div
      class="absolute bg-success top-0 right-0 p-2 rounded-tr-lg rounded-bl-lg"
      v-if="isValidate"
    >
    ...
    <div class="flex flex-col items-center">
      ...
      <div class="text-[#0F0F14] font-bold">
        {{ product.bulkPrice }} F /Casier
      </div>
    </div>
    ...
    <div class="flex items-end space-x-2" v-if="!isValidate">
      ...
      <div
        class="w-12 h-9 rounded flex items-center justify-center bg-primary text-white"
        @click="onSubmit"
      >
