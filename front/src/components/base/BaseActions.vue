      ...
      <MenuItems
        ...
        <div class="px-1 py-1 font-semibold">
          <template v-for="action in actions">
            ...
            <button data-test="action-item" @click="action.action(data)" :class="[
              active ? 'bg-gray-100' : 'text-gray-900',
              'group flex space-x-3 w-full items-center rounded-md px-2 py-2 text-sm ', action.titleClass
            ]">
               <BaseIcon :name="action.icon" class="w-6 h-6 mr-6" :class="action.iconClass"  />
