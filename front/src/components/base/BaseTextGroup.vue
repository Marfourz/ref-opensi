<template>
  <div class="my-6 sm:my-10 border-b pb-2 border-gray-200">
    <div
      class="flex items-center space-x-2 title text-lg font-bold text-primary mb-2"
      v-if="title"
    >
      <div>{{ title }}</div>
    ...
    <div
      ...
      :class="{
        'border-gray-200 bg-[#F4F6F9] rounded-lg pl-2 pt-1 pb-2': gray,
      }"
    ...
    <div class="" v-if="hasOptions">
      <div
        ...
        :key="i"
        class="my-3 sm:my-5 last:border-b-0 border-b pb-4"
        :class="{ 'border-gray-200 bg-[#F4F6F9] rounded-lg pl-2 pt-1': gray }"
      >
        <div
